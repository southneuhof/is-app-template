...
- If loading content CSS takes more than 500ms, the editor will be set to an *On Progress* state until the CSS is ready. #TINY-10008
...
### Improved
- When defining a dialog or inline dialog, if the buttons property is an empty array, or is not defined at all, the footer will now no longer be rendered. #TINY-9996
...
- Added the top right close button of dialog dialogs to the tabbing order. The 'x' button in these dialogs can now be accessed using keyboard navigation. #TINY-9520
...
- The `searchreplace` dialog closed incorrectly when clicking outside of the alert that pops up when no match is found. #TINY-9443
...
- Fixed the floating toolbar drawer height collapsing when the editor is rendered in dialog dialogs or floating containers. #TINY-4837
...
- Fixed clicking outside a dialog dialog focusing on the document body. #TINY-4249
...
### Changed
- Changed dialog dialogs to prevent dragging by default and added new `draggable_dialog` setting to restore dragging. #TINY-3873
...
- Fixed inline dialogs appearing above dialog dialogs. #TINY-3932
...
- Fixed an issue where the page could be scrolled with dialog dialogs open. #TINY-2252
...
- Fixed editor flashing unstyled during load (still On Progress). #AP-349
